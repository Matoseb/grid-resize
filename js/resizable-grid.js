  ...
  const app = new Vue({
    ...
    methods: {
      ...
      getRow(rowIndex) {
        const row = []
        for (let column = 0; column < this.columns.length; column++) {
          const cell = this.getCell({ row: rowIndex, column })
          row.push(cell)
        }
        return row
      },
      ...
      getColumn(columnIndex) {
        const column = []
        for (let row = 0; row < this.rows.length; row++) {
          const cell = this.getCell({ row, column: columnIndex })
          column.push(cell)
        }
        return column
      },
      ...
      cellMouseDown(ev) {
        ...
        if (ev.offsetY >= offsetHeight) {
          ...
          this.mouse.down = true
          const rowLeft = this.toPixelUnit(this.rows[row], true)
          const rowRight = this.toPixelUnit(this.rows[row + 1], true)
          this.mouse.rowTotal = rowLeft + rowRight
          document.body.classList.add('resize-y')
        }
      },

      insert({ column, row }, elem) {
      ...
      toPixelUnit(number, isVertical=false) {
        console.log(number)
      ...
      unlockHorizontal(gapIndex) {
        const cells = this.getRow(gapIndex)
        this.lockCells(cells, 'horizontal', false)
      },
      unlockVertical(gapIndex) {
        const cells = this.getColumn(gapIndex)
        this.lockCells(cells, 'vertical', false)
      },
      lockHorizontal(gapIndex) {
        const cells = this.getRow(gapIndex)
        this.lockCells(cells, 'horizontal')
      },
      lockVertical(gapIndex) {
        const cells = this.getColumn(gapIndex)
        this.lockCells(cells, 'vertical')
      },
      lockCells(cells, way = 'horizontal', lock = true) {
        const action = lock ? 'add' : 'remove'
        cells.forEach(({ classList }) => classList[action](`lock-${way}`))
      },
    },
